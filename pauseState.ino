...
#define LEDGR3_1 8 //LED #8
#define LEDGR3_2 10 //LED #10

void callback(char* topic, byte* payload, unsigned int length);

char *IOT_CLIENT = "d:<yourOrgUnit>:<yourDeviceType>:<yourDeviceName>"; //Device1
char *IOT_HOST = "<yourOrgUnit>.messaging.internetofthings.ibmcloud.com";  
char *IOT_PASSWORD = "<yourDeviceAPIKey>"; //Device1
    ...
    if (client.isConnected()){
        if(state==0){
            ...
            switch(ledgroup){
                case 0:
                    b.setBrightness(200);
                    b.ledOn(LEDGR0_1, 0, 255, 0);
                    b.ledOn(LEDGR0_2, 0, 255, 0);
                    RGB.color(0, 0, 255);
                ...
                case 1:
                    ...
                    break;
                case 2:
                    b.ledOn(LEDGR2_1, 255, 0, 0);
                    b.ledOn(LEDGR2_2, 255, 0, 0);
                    RGB.color(255, 0, 0);
                    plog(msg+"-Button pressed!");
                    break;
                case 3:
                    b.ledOn(LEDGR3_1, 255, 255, 0);
                    b.ledOn(LEDGR3_2, 255, 255, 0);
                    RGB.color(255, 255, 0);
                    plog(msg+"-Button pressed!");
                    break;
                default:
...
void loop() {
    if (client.isConnected()==false){
        ...
        iotConnection();
        //Sending last Msg, if Connection in past iteration was lost -> handling in iotConnection()
        msgBuffer=NULL;
    }
    stateDelayer(200);
    if (c.buttonOn(1)){
        publish("smoke", 0, 200);//Device1
        //publish("like", 0, 200);//Device2

    }else if(c.buttonOn(2)){
        publish("wc", 1, 200);//Device1

    }else if(c.buttonOn(3)){
        publish("coffee", 2, 200);//Device1
        //publish("dislike", 2, 200);//Device2
